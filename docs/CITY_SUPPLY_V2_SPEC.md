...
> 重要口径：V2 第一阶段没有 driver_id，因此以上三个字段在跨日或跨时段聚合后代表“司机人次”，不能解释为唯一司机数。唯一司机供给人次、留存和召回必须等 driver_id 级数据后再做。
...
第二阶段不新增经营指标，重点优化“10 秒看懂、30 秒定位”的使用路径。

### 12.1 主路径

页面默认阅读顺序调整为：

> 筛选与周期 → 核心经营结果 → 当前周期经营状态 → 经营异常池 → 城市/区域经营对比 → 时空供需诊断 → 核心趋势

经营结果拆解、运力效率与毛利明细下沉到折叠区域，避免首屏后的阅读路径被辅助分析打断。
...
目的：管理者无需先阅读完整异常池，即可知道当前最需要处理的问题。

### 12.4 异常池压缩

默认只展开前 3 条最高优先级异常，其余异常进入折叠区。

### 12.5 司机供给口径提示

核心 KPI 下固定说明：
- “司机供给人次”是当前事实粒度下的聚合供给人次
- 当前无 driver_id，不代表跨日去重的唯一司机数
