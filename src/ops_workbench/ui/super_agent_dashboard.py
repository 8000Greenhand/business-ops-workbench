...
def build_dashboard() -> Dashboard:
    ...
    selectors = {
        ...
        "E": agents[(agents["confidence"] < policy["potential"]["confidence_gate"]) & (agents["growth_score"] > 80)].sort_values("potential_score", ascending=False),
...
def decision_table(agents: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "优先级": agents["priority"], "首选动作": agents["primary_action"].fillna("继续观察"),
